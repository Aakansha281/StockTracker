...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self,stockpicker):
        ...
        if(len(task)>0):
            ...
            for x in stockpicker:
                if x not in args:
                    args.append(x)
            print("STOCKPICKERPASSEDTOCELERYBEAT", stockpicker)
            print("ARGS",args)
    ...
    # separate synchronous function for django ORM operation
    @sync_to_async
    ...
    async def connect(self):
        ...
        print("FROM CONSUMERS",query_params)
    ...
    async def send_stock_update(self, event):
        ...
        # since python is event driven we need to perform this step otherwise actual object value will be changed i.e. stock will be deleted for all the users
